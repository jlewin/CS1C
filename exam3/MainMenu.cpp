...
void orderMenu(PosSystem& posSystem);

// Main menu for the interactive restaurant point of sale system
int main_xxx()
{
...
void orderMenu(PosSystem &posSystem) {
    ...
    CustomerOrder* order = posSystem.addOrder(customerName);

    posSystem.printMenu();

    cout << "Add Menu Items (0 to finish)" << endl;
    do
    {
        // Report on invalid choice
        if (!choiceValid)
        ...
        case '5':
        {
            int itemIndex = choice - '0' - 1;
            int quantity;
            choiceRow("Enter quantity: ");
            cin >> quantity;

            order->addItem(posSystem.getMenuItem(itemIndex), quantity);
            break;
        }
